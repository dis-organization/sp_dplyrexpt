### R code from vignette source 'vignettes/intro_sp.Rnw'
...
dimensions(xy.sp)
xy.sp[1:2]
xy.df = as.data.frame(xy.sp) %>% as_data_frame
class(xy.df)
dim(xy.df)
summary(xy.sp)
...
###################################################
df = data.frame(z1 = round(5 + rnorm(10), 2), z2 = 20:29) %>% as_data_frame
df
...
xy.df = as.data.frame(xy.spdf) %>% as_data_frame
xy.df[1:2,]
...
###################################################
df1 = data.frame(xy, df) %>% as_data_frame
...
###################################################
df2 = data.frame(xy, df) %>% as_data_frame
coordinates(df2) = ~xc+yc
df2[1:2,]
as.data.frame(df2)[1:2,] %>% as_data_frame
as_data_frame(df2)
...
###################################################
pts = expand.grid(x = 1:3, y = 1:4, z=2:7) %>% as_data_frame
...
grd.attr = data.frame(attr, z1 = 1:9, z2 = 9:1) %>% as_data_frame
...
###################################################
df = data.frame(z = c(1,2), row.names=sapply(slot(Sl, "lines"), function(x) slot(x, "ID"))) %>% as_data_frame()
